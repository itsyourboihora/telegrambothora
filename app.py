...
def send_message(chat_id, text, with_contact_button=False):
    data = {"chat_id": chat_id, "text": text}
    if with_contact_button:
        data["reply_markup"] = (
            '{"inline_keyboard": [[{"text": "\\ud83d\\udce9 Contact Owner", "url": "https://t.me/swweeeetyapie"}]]}'
        )
...
def webhook():
    ...
    if "message" in update:
        ...
        if str(chat_id) == OWNER_CHAT_ID:
            ...
            # Auto-reply to the customer, with a Contact Owner button
            send_message(chat_id, WELCOME_MESSAGE, with_contact_button=True)
